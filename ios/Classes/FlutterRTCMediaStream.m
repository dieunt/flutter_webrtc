                             ...
                             result:(FlutterResult) result
{
   if (self.mediaRecorder) {
            result([FlutterError errorWithCode:@"StartRecordFailed" message:nil details:@"已经在录制"]);
       return;
   }
    self.mediaRecorder = [[FlutterRTCMediaRecorder alloc] initWithVideoTrack:videoTrack
                                                                  audioTrack:audioTrack
                                                                      toPath:path];
    [self.mediaRecorder startRecord: result];
}

-(void)mediaStreamTrackStopRecord:(FlutterResult) result
{
    if (self.mediaRecorder != nil) {
        [self.mediaRecorder stopRecord: result];
        self.mediaRecorder = nil;
        result(nil);
    } else {
        result([FlutterError errorWithCode:@"mediaRecorder is nil" message:nil details:nil]);
    }
}
