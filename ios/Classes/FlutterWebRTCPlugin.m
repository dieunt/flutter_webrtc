...
/*
        ...
        */
        ...
        result(FlutterMethodNotImplemented);
#endif
    } else if ([@"StopAudioMode" isEqualToString:call.method]) {  
        NSLog(@"StopAudioMode:");     
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance]; 
        [session setActive:NO error:nil];   
        result(nil);
    } else if ([@"StartAudioMode" isEqualToString:call.method]) {
        NSLog(@"StartAudioMode:");
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance]; 
        [session setActive:YES error:nil];
        result(nil);
    } else if ([@"setSpeekMute" isEqualToString:call.method]) {
        NSDictionary* argsMap = call.arguments;
        BOOL speekenable = [argsMap[@"mute"] boolValue];
         RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
         BOOL res = [session isSpeekMute];
         if(speekenable!= res){
            [session setIsSpeekMute:speekenable];
         }
        result(nil);
    } else if ([@"StartSpeek" isEqualToString:call.method]) {
        NSLog(@"StartSpeek");
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance]; 
        BOOL res = [session isSpeekMute]; 
        if(res==TRUE){
            [session setIsSpeekMute:FALSE];
        }
        
        result(nil);
    } else if ([@"StopSpeaker" isEqualToString:call.method]) {
        NSLog(@"StopSpeaker");
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];  
        [session setIsSpeekMute:TRUE];
        result(nil);
    } else if ([@"getSpeakerEnable" isEqualToString:call.method]) {
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
        BOOL res = [session isSpeekMute];
        result(@(res));
        ...
        NSDictionary* argsMap = call.arguments;
        BOOL micenable = [argsMap[@"mute"] boolValue];
         RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
         BOOL res = [session isMicrophoneMute];
         if(micenable!= res){
            [session setIsMicrophoneMute:micenable];
         }
        
        result(nil);
    } else if ([@"getSpeakerMute" isEqualToString:call.method]) {
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
        BOOL res = [session isSpeekMute];
        //NSLog(@"getSpeakerMute: %d", res);
        result(@(res));
     } else if ([@"getMicrophoneMute" isEqualToString:call.method]) {
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
        BOOL res = [session isMicrophoneMute];
        //NSLog(@"getMicrophoneMute: %d", res);
        result(@(res));
    } else if ([@"getMicrophoneEnable" isEqualToString:call.method]) {
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
        BOOL res = [session isMicrophoneMute];
        //NSLog(@"getMicrophoneEnable: %d", res);
        result(@(res));
    } else if ([@"StartMicrophone" isEqualToString:call.method]) {
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
        NSLog(@"setIsMicrophoneMute false");
        [session setIsMicrophoneMute:false];
        result(nil);
    } else if ([@"StopMicrophone" isEqualToString:call.method]) {
        RTC_OBJC_TYPE(RTCAudioSession) *session = [RTC_OBJC_TYPE(RTCAudioSession) sharedInstance];
        NSLog(@"setIsMicrophoneMute true");
        [session setIsMicrophoneMute:true];
        result(nil);
